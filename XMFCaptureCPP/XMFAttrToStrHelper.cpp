...
const std::map<const GUID, const std::wstring> s_map_UID_to_NAME =
	{
		// from XOSUtilities.h
...
const std::wstring XMFAttrToStrHelper::GUIDToAttrName(const GUID pguidKey)
{
	std::map<const GUID, const std::wstring>::const_iterator itr = s_map_UID_to_NAME.find(pguidKey);
	if (itr != s_map_UID_to_NAME.end())
	{
		return s_map_UID_to_NAME.at(pguidKey);
	}

	OLECHAR* bstrGuid;
	HRESULT hr = StringFromCLSID(pguidKey, &bstrGuid);
	if (SUCCEEDED_Xb(hr))
	{
		std::wstring retval = bstrGuid;
...
std::wstring XMFAttrToStrHelper::PropToVarType(PROPVARIANT pValue)
{
	// get type
	std::wstring  typeStr = L"WTF!!";
	switch (pValue.vt)
	{
	case VT_EMPTY:
		typeStr = L"type(VT_EMPTY(0))(nothing)";
		break;
	case VT_NULL:
		typeStr = L"type(VT_NULL(1))(SQL style Null)";
		break;
	case VT_I2:
		typeStr = L"type(VT_I2(2))(2 byte signed int)";
		break;
	case VT_I4:
		typeStr = L"type(VT_I4(3))(4 byte signed int)";
		break;
	case VT_R4:
		typeStr = L"type(VT_R4(4))(4 byte real)";
		break;
	case VT_R8:
		typeStr = L"type(VT_R8(5))(8 byte real)";
		break;
	case VT_CY:
		typeStr = L"type(VT_CY(6))(currency)";
		break;
	case VT_DATE:
		typeStr = L"type(VT_DATE(7))(date)";
		break;
	case VT_BSTR:
		typeStr = L"type(VT_BSTR(8))(OLE Automation string)";
		break;
	case VT_DISPATCH:
		typeStr = L"type(VT_DISPATCH(9))(IDispatch *)";
		break;
	case VT_ERROR:
		typeStr = L"type(VT_ERROR(10))(SCODE)";
		break;
	case VT_BOOL:
		typeStr = L"type(VT_BOOL(11))(True = -1, False = 0)";
		break;
	case VT_VARIANT:
		typeStr = L"type(VT_VARIANT(12))(VARIANT *)";
		break;
	case VT_UNKNOWN:
		typeStr = L"type(VT_UNKNOWN(13))(IUnknown *)";
		break;
	case VT_DECIMAL:
		typeStr = L"type(VT_DECIMAL(14))(16 byte fixed point)";
		break;
	case VT_RECORD:
		typeStr = L"type(VT_RECORD(36))(user defined type)";
		break;
	case VT_I1:
		typeStr = L"type(VT_I1(16))(nothing)";
		break;
	case VT_UI1:
		typeStr = L"type(VT_UI1(17))(signed char)";
		break;
	case VT_UI2:
		typeStr = L"type(VT_UI2(18))(unsigned short)";
		break;
	case VT_UI4:
		typeStr = L"type(VT_UI4(19))(ULONG)";
		break;
	case VT_I8:
		typeStr = L"type(VT_I8(20))(signed 64 - bit int)";
		break;
	case VT_UI8:
		typeStr = L"type(VT_UI8(21))(unsigned 64 - bit int)";
		break;
	case VT_INT:
		typeStr = L"type(VT_INT(22))(signed machine int)";
		break;
	case VT_UINT:
		typeStr = L"type(VT_UINT(23))(unsigned machine int)";
		break;
	case VT_INT_PTR:
		typeStr = L"type(VT_INT_PTR(37))(signed machine register size width)";
		break;
	case VT_UINT_PTR:
		typeStr = L"type(VT_UINT_PTR(38))(unsigned machine register size width)";
		break;
	case VT_VOID:
		typeStr = L"type(VT_VOID(24))(C style void)";
		break;
	case VT_HRESULT:
		typeStr = L"type(VT_HRESULT(25))(Standard return type)";
		break;
	case VT_PTR:
		typeStr = L"type(VT_PTR(26))(pointer type)";
		break;
	case VT_SAFEARRAY:
		typeStr = L"type(VT_EMPTY(27))(use VT_ARRAY in VARIANT)";
		break;
	case VT_CARRAY:
		typeStr = L"type(VT_CARRAY(28))(C style array)";
		break;
	case VT_USERDEFINED:
		typeStr = L"type(VT_USERDEFINED(29))(user defined type)";
		break;
	case VT_LPSTR:
		typeStr = L"type(VT_LPSTR(30))(null terminated string)";
		break;
	case VT_LPWSTR:
		typeStr = L"type(VT_LPWSTR(31))(wide null terminated string)";
		break;
	case VT_FILETIME:
		typeStr = L"type(VT_FILETIME(64))(FILETIME)";
		break;
	case VT_BLOB:
		typeStr = L"type(VT_BLOB(65))(Length prefixed bytes)";
		break;
	case VT_STREAM:
		typeStr = L"type(VT_STREAM(66))(Name of the stream follows)";
		break;
	case VT_STORAGE:
		typeStr = L"type(VT_STORAGE(67))(Name of the storage follows)";
		break;
	case VT_STREAMED_OBJECT:
		typeStr = L"type(VT_STREAMED_OBJECT(68))(Stream contains an object)";
		break;
	case VT_STORED_OBJECT:
		typeStr = L"type(VT_STORED_OBJECT(69))(Storage contains an object)";
		break;
	case VT_VERSIONED_STREAM:
		typeStr = L"type(VT_VERSIONED_STREAM(73))(Stream with a GUID version)";
		break;
	case VT_BLOB_OBJECT:
		typeStr = L"type(VT_BLOB_OBJECT(70))(Blob contains an object)";
		break;
	case VT_CF:
		typeStr = L"type(VT_CF(71))(Clipboard format)";
		break;
	case VT_CLSID:
		typeStr = L"type(VT_CLSID(72))(A Class ID)";
		break;
	case VT_VECTOR:
		typeStr = L"type(VT_VECTOR(4096))(simple counted array)";
		break;
	case VT_ARRAY:
		typeStr = L"type(VT_ARRAY(8192))(SAFEARRAY *)";
		break;
	case VT_BYREF:
		typeStr = L"type(VT_BYREF(16384))(void* for local use)";
		break;
	case VT_BSTR_BLOB:
		typeStr = L"type(VT_BSTR_BLOB(4095))(Reserved for system use)";
		break;
	case 4113:
		typeStr = L"type(UNKNOWN(4113))(BYTE[])";
		break;
	default:
		typeStr = L"type(UNKNOWN(" + std::to_wstring(pValue.vt) + L"))";
		break;
	}
	return typeStr;
}

std::wstring XMFAttrToStrHelper::PropToValueImp(CComPtr<IMFAttributes> pAttributes, GUID pguidKey, PROPVARIANT pValue)
{
	// get type
	std::wstring  valStr = L"WTF!!";
	switch (pValue.vt)
	{
	case VT_EMPTY:
		valStr = L"empty";
		break;
	case VT_NULL:
		valStr = L"not implemented";
		break;
	case VT_I2:
		valStr = std::to_wstring(pValue.iVal);
		break;
	case VT_I4:
		valStr = std::to_wstring(pValue.lVal);
		break;
	case VT_R4:
		valStr = std::to_wstring(pValue.fltVal);
		break;
	case VT_R8:
		valStr = std::to_wstring(pValue.dblVal);
	case VT_CY:
		valStr = L"not implemented";
	case VT_DATE:
		valStr = std::to_wstring(pValue.date);
		break;
	case VT_BSTR:
		valStr = L"not implemented";
		break;
	...
	case VT_VARIANT:
		...
		break;
	case VT_UNKNOWN:
		if (pguidKey == MFSampleExtension_Token)
		{ // this is a Sample
			valStr = L"Token";
		}
		else
	...
	case VT_RECORD:
		...
		break;
	case VT_I1:
		valStr = std::to_wstring(pValue.cVal);
		break;
	case VT_UI1:
		valStr = std::to_wstring(pValue.bVal);
		break;
	case VT_UI2:
		valStr = std::to_wstring(pValue.iVal);
		break;
	case VT_UI4:
		if (pguidKey == MFSampleExtension_Discontinuity)
		...
		{
			valStr = std::to_wstring(pValue.ulVal);
		}
		break;
	case VT_I8:
		valStr = std::to_wstring(pValue.iVal);
	...
	case VT_UI8: 
	{
		if (pguidKey == MFSampleExtension_DeviceTimestamp)
		{
			valStr = std::to_wstring(pValue.hVal.QuadPart);
		}
		else
		{
			...
			HRESULT hr = MFGetAttributeRatio(pAttributes, pguidKey, &numerator, &denominator);
			if (SUCCEEDED_Xb(hr))
			{
				valStr = std::to_wstring(numerator) + L"/" + std::to_wstring(denominator);
			}
		}
	...
	case VT_INT:
		valStr = std::to_wstring(pValue.intVal);
		break;
	case VT_UINT:
		valStr = std::to_wstring(pValue.uintVal);
		break;
	case VT_INT_PTR:
		valStr = L"not implemented";
		break;
	...
	case VT_LPWSTR:
	{
		std::wstring sws = pValue.pwszVal;
		valStr = sws;
		break;
	}
	case VT_FILETIME:
	...
	case VT_CLSID:
	{
		valStr = GUIDToAttrName(*(pValue.puuid));
		break;
	}
	case VT_VECTOR:
		valStr = L"not implemented";
	...
	case 4113:
	{
		...
		UINT8* pBuf = NULL;
		UINT32 bufSize = 0;
		HRESULT hr = pAttributes->GetAllocatedBlob(MF_DEVSOURCE_ATTRIBUTE_MEDIA_TYPE, &pBuf, &bufSize);
		if (SUCCEEDED(hr))
		{
			if (bufSize == 32)
			{
				char* charMAJOR = ((char*) (pBuf));
				char* charMINOR = ((char*) (pBuf + 16));
				...
				valStr = L"MAJOR-" + wsMAJOR + L" MINOR-" + wsMINOR;
			}
		}
		else
		{
	...
	default:
		valStr = std::to_wstring(pValue.vt) + L" unknown";
		break;
	}
	return valStr;
}

std::wstring XMFAttrToStrHelper::MediaTypeToValue(CComPtr<IMFActivate> device)
{
	
	UINT32 cbBlob = 0;
	HRESULT hr = device->GetBlobSize(MF_DEVSOURCE_ATTRIBUTE_MEDIA_TYPE, &cbBlob);
	BYTE *pBlob = NULL;
	if (SUCCEEDED_Xb(hr))
	{
	...
	hr = device->GetBlob(MF_DEVSOURCE_ATTRIBUTE_MEDIA_TYPE, pBlob, cbBlob, &cbBlob);
