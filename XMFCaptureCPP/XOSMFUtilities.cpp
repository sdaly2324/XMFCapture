...
#include "XOSMFAttrToStrHelper.h"
...
void DumpAttrImp(CComPtr<IMFAttributes> attrs, const std::wstring& typeName, const std::wstring& name)
{
	...
	if (SUCCEEDED_XOSb(hr))
	{
		for (unsigned int x = 0; x < pcItems; x++)
		{
			...
			{
				// get attribute name
				std::wstring attrName = XOSMFAttrToStrHelper::GUIDToAttrName(pguidKey);
				std::wstring attrVal = L"UNKOWN";

				attrVal = XOSMFAttrToStrHelper::PropToValue(attrs, pguidKey, pValue);
